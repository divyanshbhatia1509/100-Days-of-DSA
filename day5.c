...
Explanation: Compare the next unprocessed arrival time from both logs and append the earlier one 
to the final log until all entries are processed*/
...
int main() {
    ...
    // Print remaining elements of log 1 (if any)
    while (i < p) {
        printf("%d ", a[i]);
        i++;
    }

    // Print remaining elements of log 2 (if any)
    while (j < q) {
        printf("%d ", b[j]);
        j++;
    }
